...
export class Game {
     ...
     * クライアントからの受信
     */
    public onFuro(kaze: Types.kaze_number, furoHai: number[]): boolean{
        if(!this.responseCheck(kaze, "furo")) return false
        ...
        const check = this.furoCheck(kaze, furoHai)
        for(let k of num){
            if(k === kaze) continue
            if(this.players[this.jika[k]] === "game"){
                this.server.getProtocol().emit("furo", this.jika[k], {"protocol": "furo", "kaze": kaze, "hai": furoHai})
            }
        }
     ...
     * @returns string[]
     */
    private furoCheck(kaze: Types.kaze_number, furoHai: number[]): string[]{
        return ["dahai"]
    }
    ...
    public onKaikan(kaze: Types.kaze_number, kanHai: number[]): boolean{
        if(!this.responseCheck(kaze, "kaikan")) return false
        this.updateJunhai()
