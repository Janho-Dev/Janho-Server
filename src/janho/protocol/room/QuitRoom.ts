...
export class QuitRoom implements JanhoProtocol {
    ...
    public procReceive(socketId: string, data: string): void{
        ...
        if(room !== null){
            const result = room.quit(socketId)
            if(result) this.procEmit(socketId, {"protocol": "quitRoom", "result": true})
            else this.procEmit(socketId, {"protocol": "quitRoom", "result": false})
        }
    }
