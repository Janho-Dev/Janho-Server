...
export class Register implements JanhoProtocol {
    ...
    public procReceive(socketId: string, data: string): void{
        ...
        if("name" in parsed){
            if(typeof parsed["name"] === "string"){
                const result = this.server.addUser(socketId, parsed["name"])
                if(result) this.procEmit(socketId, {"protocol": "register", "result": true})
                else this.procEmit(socketId, {"protocol": "register", "result": false})
            }
        }
    }
